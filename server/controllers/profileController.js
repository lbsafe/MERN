...
exports.find = async (req, res, next) => {
    try{
        ...
        // 조건1 - 특정 유저가 팔로우 하는 프로필 목록
        if ("following" in req.query) {
            // req.query: URL에 담아 서버에 전송하는 작은 데이터 (쿼리스트링 ?키 = 값)

            // 특정 유저를 검색한다.
            const user = await User
                .findOne({ username: req.query.following });

            // 유저가 존재하지 않을 경우 404 에러
            if(!user){
        ...
        if("username" in req.query){
            ...
            // 정규식에서 i - insensitve (대소문자를 가리지 않는다.)
...
exports.follow = async (req, res, next) => {
    try{
        ...
        if(req.user.username === req.params.username){
            throw new createError.BadRequest("Cannot follow yoerself");
        }
